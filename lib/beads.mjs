...
const writtenCapabilityCacheFiles = new Set();

export function clearCapabilityCache() {
  capabilityCache.clear();
  for (const path of writtenCapabilityCacheFiles) {
    try {
      rmSync(path, { force: true });
    } catch {
      // Best-effort: a file we can no longer remove is not a correctness issue.
    }
  }
  writtenCapabilityCacheFiles.clear();
}

// Capabilities belong to the `bd` binary, not the project, so the on-disk
// entry lives once per Git common dir (shared by every worktree) rather than
// per-repo state. Any failure here — no worktree, corrupt JSON, a stale key —
// is a cache miss, never a command failure: capabilities are always
// re-probed on error.
function capabilityCacheFilePath(cwd, cacheKey) {
  const root = join(gitCommonDirectory(cwd), 'sdlc', 'capabilities');
  const hash = createHash('sha256').update(cacheKey).digest('hex');
  return join(root, `${hash}.json`);
}

function readDiskCapabilityCache(cwd, cacheKey) {
  try {
    const path = capabilityCacheFilePath(cwd, cacheKey);
    const record = JSON.parse(readFileSync(path, 'utf8'));
    return record.key === cacheKey ? record.result : undefined;
  } catch {
    return undefined;
  }
}

function writeDiskCapabilityCache(cwd, cacheKey, result) {
  try {
    const path = capabilityCacheFilePath(cwd, cacheKey);
    mkdirSync(dirname(path), { recursive: true });
    const temporary = `${path}.${process.pid}.${randomUUID()}.tmp`;
    try {
      writeFileSync(temporary, JSON.stringify({ key: cacheKey, result }), { mode: 0o600 });
      renameSync(temporary, path);
    } finally {
      rmSync(temporary, { force: true });
    }
    writtenCapabilityCacheFiles.add(path);
  } catch {
    // No Git worktree, no permissions, disk full: the cache is optional.
  }
}

// Pure fs lookup, mirroring how the shell would find `executable` on PATH.
// Must never spawn a process — the whole point is to key the cache without
// paying for a `which`/`command -v` call.
function resolveExecutableForCache(executable) {
  if (executable.includes('/') || executable.includes(sep)) {
    const resolvedPath = resolve(executable);
    try {
      return { resolvedPath, mtimeMs: statSync(resolvedPath).mtimeMs };
    } catch {
      return { resolvedPath: '', mtimeMs: 0 };
    }
  }
  for (const directory of String(process.env.PATH ?? '').split(delimiter)) {
    ...
    try {
      ...
      if (stat.isFile()) return { resolvedPath: candidate, mtimeMs: stat.mtimeMs };
    } catch {
      // Not on this PATH entry; keep scanning.
    }
  }
  return { resolvedPath: '', mtimeMs: 0 };
}
...
export function inspectBeadsInstallation({ cwd = process.cwd(), executable = 'bd', runner = defaultCommandRunner } = {}) {
  const cacheable = runner === defaultCommandRunner;
  ...
  let cacheKey;
  if (cacheable) {
    const { resolvedPath, mtimeMs } = resolveExecutableForCache(executable);
    cacheKey = `${executable}\0${resolvedPath}\0${mtimeMs}\0${versionStdout}`;
    const cached = capabilityCache.get(cacheKey);
    if (cached) return structuredClone(cached);
    const fromDisk = readDiskCapabilityCache(cwd, cacheKey);
    if (fromDisk) {
      capabilityCache.set(cacheKey, fromDisk);
      return structuredClone(fromDisk);
    }
  }

  const parsed = parseBeadsVersion(versionStdout);
  ...
  if (cacheable) {
    ...
    writeDiskCapabilityCache(cwd, cacheKey, result);
...
export function createBeadsAdapter({
  ...
  return {
    ...
    listInProgress() {
      return collectionFromBeadsJson(readonlyJson(['list', '--status=in_progress', '--limit', '0']));
    },
    claim(id) {
      return mutateJson(['update', id, '--claim']);
    },
    appendNote(id, text) {
      return mutateJson(['note', id, text]);
    },
    ...
    listGates({ all = false } = {}) {
      ...
      return collectionFromBeadsJson(readonlyJson(args)).map((gate) => {
