...
export function parsePlan(contents, { path, ticket } = {}) {
  ...
  const isolationValue = frontmatter.attributes.Isolation;
  const isolationValid = ['branch', 'worktree'].includes(isolationValue);
  if (isolationValue && !isolationValid) errors.push('Plan has an invalid Isolation.');
  const isolation = isolationValid ? isolationValue : 'branch';
