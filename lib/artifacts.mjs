...
export function parseTicket(contents, { path } = {}) {
  ...
  if (!['feature', 'bug', 'refactor', 'chore'].includes(frontmatter.attributes.Type)) errors.push('Ticket has an invalid or missing Type.');
...
export function parsePlan(contents, { path, ticket } = {}) {
  ...
  if (!['feature', 'bug', 'refactor', 'chore'].includes(frontmatter.attributes.Type)) errors.push('Plan has an invalid or missing Type.');
