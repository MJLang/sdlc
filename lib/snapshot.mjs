...
function recoveryFor(diagnosis) {
  if (diagnosis.state === 'ready_for_approval' || diagnosis.state === 'reapproval_required') return `/approve ${diagnosis.number}`;
...
function humanEntries(diagnosis) {
  ...
  if (diagnosis.state === 'ready_for_approval') entries.push({ number: diagnosis.number, code: 'approval', action: `/approve ${diagnosis.number}` });
  if (diagnosis.state === 'reapproval_required') entries.push({ number: diagnosis.number, code: 'reapproval', action: `/approve ${diagnosis.number}` });
  if (diagnosis.state === 'legacy') entries.push({ number: diagnosis.number, code: 'legacy', action: recoveryFor(diagnosis) });
  if (diagnosis.review?.valid && /^APPROVED/.test(diagnosis.review.verdict ?? '')) entries.push({ number: diagnosis.number, code: 'land', action: `/land ${diagnosis.number}` });
