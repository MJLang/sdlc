...
// Adding `config` below is additive-only - every existing key keeps its name,
// position, and meaning, and a consumer that does not know the key simply
// ignores it - so it does not warrant a schema version bump.
...
export function buildSnapshot({ view, context, diagnoses, actor = null, now = context.now ?? Date.now() }) {
  ...
  const base = {
    ...
    // `context.config` is already collected once by `createDoctorInspectionContext`;
    // projecting it here introduces no new read (AC-003).
    config: configProjection(context.config),
    beads: compact({
