...
function configValue(contents, label) {
  const escaped = label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const pattern = '^-[ \\t]+\\*\\*' + escaped + ':\\*\\*[ \\t]*(?:`([^`]*)`|([^<\\n]*))';
  return contents.match(new RegExp(pattern, 'mi'))?.slice(1).find((value) => value !== undefined)?.trim();
}

function projectBeadsConfig(root) {
  const path = join(root, 'thoughts', 'AGENTS.md');
  const contents = existsSync(path) ? readFileSync(path, 'utf8') : '';
  const mode = configValue(contents, 'Beads mode');
  const slot = configValue(contents, 'Beads merge slot');
  const errors = [];
  if (mode && !['embedded', 'server'].includes(mode)) errors.push(`Project Configuration has invalid Beads mode ${JSON.stringify(mode)}.`);
  if (slot && !['off', 'on'].includes(slot)) errors.push(`Project Configuration has invalid Beads merge slot ${JSON.stringify(slot)}.`);
  return {
    mode: mode === 'server' ? 'server' : 'embedded',
    mergeSlotEnabled: slot === 'on',
    errors,
  };
}
...
function nativeDiagnosticErrors(native) {
  const errors = [];
  const warnings = [];
  ...
  for (const [name, result] of Object.entries({ context: native.context, ready: native.ready, cycles: native.cycles, worktrees: native.worktrees, gates: native.gates, escalations: native.escalations, stale: native.stale, orphans: native.orphans })) {
    if (!result.ok) errors.push(`Beads ${name} diagnostics failed: ${result.error}`);
  }
...
export function inspectDoctor(number, {
  ...
  try {
    ...
    return {
      ...
      review: null,
      errors: [`Git repository could not be resolved: ${error.message}`],
      warnings,
    };
  }
  ...
  const installation = inspectBeadsInstallation({ cwd: primary, executable: beadsExecutable, ...(beadsRunner ? { runner: beadsRunner } : {}) });
  if (!installation.available) errors.push(...installation.errors);
  else if (!installation.coreCapabilitiesValid) errors.push(...installation.errors);

  const config = projectBeadsConfig(primary);
  ...
  if (installation.coreCapabilitiesValid) {
    adapter = createBeadsAdapter({ cwd: primary, executable: beadsExecutable, ...(beadsRunner ? { runner: beadsRunner } : {}) });
    native = collectNativeDiagnostics(adapter, config);
    const nativeMessages = nativeDiagnosticErrors(native);
  ...
  if (native && mapping) {
    ...
    if (stale.length) {
      const corroborated = !worktree || (!worktree.dirty && worktree.lastCommitAt && now / 1000 - worktree.lastCommitAt >= 86400);
  ...
  if (native?.mergeSlot?.data) {
    ...
    mergeSlot = {
      ...
      updatedAt,
      ageSeconds: Number.isNaN(updatedAtMilliseconds) ? null : Math.max(0, Math.floor((now - updatedAtMilliseconds) / 1000)),
