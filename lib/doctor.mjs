...
import { parseDiscoveryResult, parsePlan, parseTicket } from './artifacts.mjs';
import { collectNativeDiagnostics, createBeadsAdapter, inspectBeadsInstallation, issueMetadata } from './beads.mjs';
import { configProjection, readProjectConfig } from './config.mjs';
...
// Ticket/plan text must come from `main`, not the primary checkout's working
// copy: in branch mode the primary checkout sits on the plan branch, so its
// filesystem holds the branch copy while approval hashes are always taken
// against main. Worktree-mode users are unaffected — their primary checkout
// is always on main and takes the readFileSync path unchanged.
function canonicalArtifactBytes(root, path) {
  if (git(root, ['rev-parse', '--abbrev-ref', 'HEAD'], { allowFailure: true }) === 'main') return readFileSync(path);
  const fromMain = commitFile(root, 'main', posixPath(relative(root, path)));
  return fromMain !== undefined ? fromMain : readFileSync(path);
}
...
export function evaluateArtifactState({ ticket, plan, ticketSha256, planSha256, approval, approvalRecordsExist = false }) {
  ...
  if (!plan) {
    if (ticket.frontmatter.Type === 'chore') {
      return { state: 'blocked', errors: ['Chore-lane ticket intentionally has no plan; resume or recover it through /sdlc-chore, not /sdlc-plan.'] };
    }
...
export function inspectDoctor(number, {
  ...
  const errors = [];
  const warnings = [];
  const shared = inspectionContext ?? createDoctorInspectionContext({ cwd, beadsExecutable, beadsRunner, now, ...(actor !== undefined ? { actor } : {}) });
  // Config (and, through it, legacy-configuration) errors are the most
  // actionable signal a caller can act on, so they lead `result.errors` even
  // though ticket/plan parsing below adds its own errors first into this
  // array. The later `errors.push(...shared.errors)` still carries every
  // other shared diagnostic (installation, native) at its established
  // position; `uniqueMessages` keeps only this first occurrence of each one.
  errors.push(...shared.config.errors);
  const primary = shared.primary;
  const inspectionNow = shared.now ?? now;
  const currentActor = actor !== undefined ? actor : shared.actor;
  if (!primary) {
    return {
      ...
      review: null,
      config: configProjection(shared.config),
  ...
  if (ticketPath) {
    try {
      const source = canonicalArtifactBytes(primary, ticketPath);
  ...
  if (planPath) {
    try {
      const source = canonicalArtifactBytes(primary, planPath);
      planSource = source.toString('utf8');
      plan = parsePlan(planSource, { path: posixPath(relative(primary, planPath)), ticket });
  ...
  if (installation.coreCapabilitiesValid && adapter) {
    ...
    if (epic && plan) {
      const details = new Map(children.map((child) => [issueId(child), child]));
  ...
  if (['reapproval_required', 'legacy'].includes(artifactState.state) && plan?.status === 'approved') {
    warnings.push(...approvalSyncErrors.map((error) => `Pending /sdlc-approve sync: ${error}`));
  } else {
  ...
  let discovery = null;
  if (plan) {
    workingTree = resolveWorkingTree(plan, { primary, worktrees: native?.worktrees?.data ?? [], cwd: primary });
    const path = workingTree.path;
    const gitState = workingTree.git;
    if (workingTree.mode === 'worktree') {
      const beadsState = workingTree.beadsState;
      ...
      if (gitState?.branch !== workingTree.planName) errors.push(`Plan worktree branch is ${gitState?.branch}, expected ${workingTree.planName}.`);
      ...
      worktree = { path, ...gitState, beadsState, snapshotMatchesApprovedPlan, snapshotSeverity: snapshotMatchesApprovedPlan === false ? 'warning' : null };
    } else if (workingTree.onPlanBranch) {
      worktree = { path, ...gitState, beadsState: null, snapshotMatchesApprovedPlan: null, snapshotSeverity: null };
    }

    // Tree-state observations belong to this plan's own working tree. In branch
    // mode a primary checkout sitting on main is nobody's working tree, so its
    // dirtiness says nothing about this plan.
    if (worktree) {
      if (ticket?.frontmatter.Type === 'discovery') {
        const reportPath = join(path, 'thoughts', 'designs', `${normalizedNumber}-discovery.md`);
        if (!existsSync(reportPath)) {
          discovery = { path: posixPath(relative(path, reportPath)), outcome: null, valid: false, errors: ['Discovery result artifact is missing.'] };
        } else {
          const parsed = parseDiscoveryResult(readFileSync(reportPath, 'utf8'), { path: posixPath(relative(path, reportPath)), ticket, plan, ticketSha256, planSha256 });
          discovery = { path: parsed.path, outcome: parsed.outcome, valid: parsed.valid, errors: parsed.errors };
        }
      }
  ...
  if (epic && issueStatus(epic) === 'in_progress' && workingTree?.mode === 'branch' && !workingTree.onPlanBranch) {
    warnings.push(`Plan branch ${workingTree.planName} is not checked out in the primary checkout (currently ${workingTree.git?.branch ?? 'unknown'}); switch to it to continue.`);
  }
  ...
  if (native && mapping) {
    ...
    const stale = native.stale.data
      .filter((issue) => planIssueIds.has(issueId(issue)))
      .filter((issue) => issueId(issue) !== issueId(epic))
      .filter((issue) => !currentActor || (issue.assignee ?? null) !== currentActor);
    if (stale.length) {
      const dirty = Boolean(workingTree?.git?.dirty);
      const unpushed = workingTree?.git?.unpushed ?? 0;
      const risky = dirty || unpushed > 0;
      if (risky) {
        const conflicts = [];
        if (dirty) conflicts.push('the working tree is dirty');
        if (unpushed > 0) conflicts.push(`${unpushed} unpushed commit${unpushed === 1 ? '' : 's'} exist`);
        errors.push(`Plan has ${stale.length} stale in-progress Beads issue${stale.length === 1 ? '' : 's'} corroborated by an unresolved conflict: ${conflicts.join(' and ')}.`);
      } else {
        warnings.push(`Beads reports ${stale.length} candidate stale in-progress Beads issue${stale.length === 1 ? '' : 's'}, but the working tree is clean with nothing unpushed; run \`sdlc resume ${normalizedNumber}\` to adopt the claim.`);
      }
    }
  ...
  const staleClaims = native?.stale?.data?.filter((issue) => children.map(issueId).includes(issueId(issue))
    && (!currentActor || (issue.assignee ?? null) !== currentActor)) ?? [];
