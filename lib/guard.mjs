...
import { configProjection } from './config.mjs';
...
function refusal(stage, diagnosis, code, detail, exitCode) {
  const primary = { code, detail, recovery: recovery(stage, diagnosis.number, code) };
  const context = (Array.isArray(diagnosis.errors) ? diagnosis.errors : [])
    .filter((error) => error !== detail)
    .map((error) => ({ code: 'context', detail: error, recovery: `sdlc doctor ${diagnosis.number} --json` }));
  return {
    ok: false,
    stage,
    ...
    errors: [primary, ...context],
  };
}

function accepted(stage, diagnosis, mode, fields = {}, extraWarnings = []) {
  // `implement`/`land` are the two stages that act on Beads coordination
  // mode directly (claim behavior, merge-slot enforcement), so their
  // accepted line carries it - both scalars, so the default output stays
  // exactly one line.
  let coordination = {};
  if (['implement', 'land'].includes(stage)) {
    const config = resolvedConfig(diagnosis);
    coordination = { mergeSlot: config.mergeSlotEnabled ? 'on' : 'off', beadsMode: config.beadsMode };
  }
  return {
    ok: true,
    exitCode: 0,
...
// `inspectDoctor` carries the resolved config as an enumerable `config` key;
// hand-built diagnoses in tests instead set the (pre-existing) non-enumerable
// `inspection.context.config` shape `createDoctorInspectionContext` produces.
// Normalizing either through `configProjection` gives every caller below the
// same defaulted shape (arrays/objects never undefined, `errors` always an
// array) regardless of which one a given diagnosis carries.
function resolvedConfig(diagnosis) {
  return configProjection(diagnosis.config ?? diagnosis.inspection?.context?.config ?? {});
}

// AC-005/T1.6: every remaining config error rides along as `context`, in
// full, so a caller never has to make a second call to see what else is
// wrong. All entries point at `sdlc config` - the one place that reports
// every effective setting and every error the same way doctor and guard do.
function configRefusal(stage, diagnosis, configErrors) {
  const [detail, ...rest] = configErrors;
  return {
    ok: false,
    stage,
    number: diagnosis.number,
    state: diagnosis.state,
    exitCode: doctorExitCode(diagnosis) || 3,
    errors: [
      { code: 'config-invalid', detail, recovery: 'sdlc config' },
      ...rest.map((error) => ({ code: 'context', detail: error, recovery: 'sdlc config' })),
    ],
  };
}

// `implement`/`land` classify changed files against declared target scopes
// (`classifyTargetPath`), so their config projection carries the map a
// caller needs to do that itself: each target's own paths, gates, and
// reviewers.
function targetMap(config) {
  const map = {};
...
/**
 * Stage-scoped slice of the resolved config, attached to every guard result
 * under `config` (the one-line formatter reads only `fields`/`errors`, so it
 * never sees this key). Each stage gets only what its own skill call
 * currently needs a second `sdlc config` read for (AC-003):
 * - `plan` authors gates before any target classification runs against
 *   changed files, so it needs the whole gate picture up front - every
 *   target name (declaration order is precedence) and each one's own gates
 *   alongside the global gates it inherits - plus the two free-text fields a
 *   plan reviewer consults.
 * - `implement`/`land` run against a live worktree whose changed files must
 *   be classified into targets, so they get the full per-target map plus the
 *   coordination fields (`beads`) that decide claim/merge-slot behavior.
 * - `approve`/`review` gate purely on doctor/Beads state - plan hash, review
 *   validity - and never run a gate command or classify a path themselves,
 *   so the coordination mode is the only field worth carrying instead of a
 *   second call.
 */
export function stageConfigProjection(stage, config) {
  const beads = { mode: config.beadsMode, mergeSlot: config.mergeSlotEnabled ? 'on' : 'off' };
  if (stage === 'plan') {
    return {
      targets: config.targets,
      gates: { global: config.qualityGates, byTarget: config.targetGates },
      productDocs: config.productDocs,
      frontendConstraints: config.frontendConstraints,
    };
  }
  if (stage === 'implement' || stage === 'land') {
    return {
      gates: { global: config.qualityGates, byTarget: config.targetGates },
      beads,
      targets: targetMap(config),
    };
  }
  return { beads };
}

function evaluateStageMatrix(stage, diagnosis, {
  ...
  const source = planSource ?? diagnosis.inspection?.planSource ?? (diagnosis.plan?.path ? readFileSync(join(diagnosis.primaryCheckout, diagnosis.plan.path), 'utf8') : '');
...
/**
 * AC-005: refuses before any stage matrix runs when the resolved config
 * carries errors - a legacy `## Project Configuration` section included,
 * since `readProjectConfig` prepends that as a config error. Every other
 * refusal and acceptance is unchanged; every result additionally carries a
 * stage-scoped `config` projection (`stageConfigProjection`) that the
 * one-line formatter ignores.
 */
export function evaluateGuard(stage, diagnosis, options = {}) {
  ...
  const config = resolvedConfig(diagnosis);
  const result = config.errors.length ? configRefusal(stage, diagnosis, config.errors) : evaluateStageMatrix(stage, diagnosis, options);
  return { ...result, config: stageConfigProjection(stage, config) };
}
...
export function inspectGuard(stage, number, {
  ...
  if (stage === 'land' && context.adapter) {
    try {
      ...
      // This is the one refusal raised outside `evaluateGuard`, so it has to
      // attach the stage-scoped projection itself or `--json` consumers would
      // see the `config` key vanish for exactly this failure.
      const refused = refusal(stage, diagnosis, 'gate-history-unavailable', error.message, 3);
      return { ...refused, config: stageConfigProjection(stage, resolvedConfig(diagnosis)) };
    }
  }
