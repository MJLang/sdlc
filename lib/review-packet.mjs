...
function reviewerMap(config, classified) {
  ...
  for (const target of config.targets) {
    const reviewers = config.reviewers[target] ?? [];
...
// The derived reviewer set alone, without building the packets around it.
// `reviewerMap` reads only each file's target lanes, so this needs one `git
// diff --name-only` — not the per-file blob reads, interface graph and lane
// diffs a full `createReviewPackets` pays for and would then discard.
export function reviewerNamesFor({ cwd = process.cwd(), base = 'main', head = 'HEAD' } = {}) {
  ...
  const classified = changedFiles(cwd, base, head).map((path) => ({
    path,
    targets: laneMapConfigured ? classifyTargetPath(path, config, pathMatchesScope) : [],
  }));
  return [...reviewerMap(config, classified).keys()].sort((left, right) => left.localeCompare(right));
}
